...
// In AI Studio, the environment automatically injects process.env.GEMINI_API_KEY
...
export async function analyzeStyle(texts: string[]): Promise<StyleProfile> {
  ...
  try {
    ...
    if (!text) {
      throw new Error("No response text returned from Gemini AI");
    }
    ...
    throw new Error(error instanceof Error ? error.message : "Failed to analyze style");
  }
}
...
export async function humanizeText(text: string, profile: StyleProfile): Promise<string> {
  ...
  try {
    ...
    throw new Error(error instanceof Error ? error.message : "Failed to humanize text");
  }
}
